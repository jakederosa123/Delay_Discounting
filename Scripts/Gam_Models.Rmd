...
model = gam(TC1 ~ Group + s(Ages, by = Group, bs = "ts", k =3), method = "ML", data = adhd_combined_nt)
summary(model)
...
ggplot(data= adhd_combined_nt, mapping = aes(fill = Group, y=TC1, x=Ages, color=Group)) + 
  ...
  theme(axis.title.x = element_text(size=16, face="bold",),
        axis.title.y = element_text(size=16, face="bold",),
        title = element_text(size=14, face="bold",)) + 
  geom_point(alpha = .4)+ facet_wrap(~Sex)  +  theme(legend.key = element_blank(), strip.background = element_rect(fill="white"), strip.text = element_text(size = 16, face = "bold"))
...
model = gam(TC1 ~ Group + Sex + s(Ages, by = Group, bs = "ts", k=3), method = "REML", data = anxiety_nt)
summary(model)
...
summary(aov(TC1 ~ Group + Ages + Sex, data = autism_nt))
```
...
ggplot(data= learning_nt, mapping = aes(fill = Group, y=TC1, x=Ages, color=Group)) + 
  ...
  theme(legend.title = element_text(size = 14, face = "bold",), 
    legend.text = element_text(size = 12, face = "bold",)) + 
   scale_fill_manual(values = c("steelblue4","red")) + 
...
summary(aov(TC1 ~ Group*Ages*Sex, data = learning_nt))
```


```{r}

df_fa = df %>% select(URSI, TC1, FA2, TC1)
...
model = gam(TC1 ~ cluster + s(Age, by = cluster, bs = "ts",  k=3), method = "REML", data = Split_1)
summary(model)
